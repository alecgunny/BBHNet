...
from aframe.architectures.resnet import convN, get_norm_layer
...
class XylophoneResidualBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        stride: int = 1,
        dilation: int = 1,
        norm_layer: torch.nn.Module = torch.nn.BatchNorm1d,
    ):
        super().__init__()
...
class Xylophone(torch.nn.Module):
    ...
    def __init__(self, num_ifos, norm_groups: Optional[int] = None):
        super().__init__()
        ...
        self.fc = torch.nn.Linear(self.out_channels, 1)
