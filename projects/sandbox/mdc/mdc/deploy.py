...
def main(
    ...
    with h5py.File(data_dir / "background.hdf", "r") as f:
        for timestamp, dataset in f[ifos[0]].items():
            sizes[timestamp] = len(dataset)
    datasets = sorted(sizes, key=lambda k: sizes[k])[::-1]
    dataset_map = [[] for _ in range(num_clients)]
    for i, dataset in enumerate(datasets):
        j = i % num_clients
        dataset_map[j].append(dataset)

    parameters = "seq_id,datasets\n"
    for i, datasets in enumerate(dataset_map):
        seq_id = sequence_id + i
        datasets = " ".join(map(str, datasets))
        parameters += f"{seq_id},{datasets}\n"
